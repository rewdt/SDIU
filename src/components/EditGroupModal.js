...
function EditGroupModal({ open, handleClose, reloadData, params, token }) {
  ...
  const [inputValues, setInputValues] = useReducer(
    (state, newState) => ({ ...state, ...newState }),
    {
      ...
      state: params.state ? params.state.id : "",
      lga: params.lga ? params.lga.id : ""
    }
  );
  ...
  const addGroup = async (data) => {
    setLoading(true);
    await Api(token)
      .updateGroup(params.id, data)
      .then((res) => {
        console.log(res.data);
        if (res.data.status === "success") {
          // console.log(res.data);
          alert.success(res.data.message, { position: "top center" });
          handleClose();
          reloadData();
        } else {
